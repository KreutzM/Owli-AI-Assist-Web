...
const session = (): WebBootstrapResponseV2 => ({
  sessionToken: 'private', expiresAt: new Date(Date.now() + 120_000).toISOString(),
  featureFlags: { sceneDescribe: false, followup: false },
  bootstrapInfo: { environment: 'staging', sessionTtlSeconds: 120, sessionSchemaVersion: 2, platform: 'web',
...
describe('401 retry loop bound', () => {
  it('surfaces a second 401 without another bootstrap', async () => {
    ...
    const operation = vi.fn(async () => { throw new RemoteHttpError(401); });
